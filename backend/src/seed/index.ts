...
async function seed() {
  ...
  await SubsidyPolicy.deleteMany({});

  console.log("创建助餐点...");
  const canteens = await Canteen.insertMany(canteenData);
  console.log(`  已创建 ${canteens.length} 个助餐点`);

  console.log("创建默认补贴政策...");
  const now = new Date();
  const defaultPolicy = new SubsidyPolicy({
    name: "社区助餐补贴政策",
    version: 1,
    ...
    description: "默认社区助餐补贴政策",
  });
  await defaultPolicy.save();
  console.log("  已创建默认补贴政策");
  ...
  for (let dayOffset = 0; dayOffset < 30; dayOffset++) {
    ...
    for (let i = 0; i < elderlyList.length; i++) {
      ...
      const canteen =
        canteens[elderly.canteenId.toString().slice(-1) === "0" ? 0 : i % 3];
      ...
      if (status === "completed") {
        ...
        const matchedRuleSnapshot = subsidy.matchedRule
          ? ruleToMatchedRule(subsidy.matchedRule)
          : {
              elderlyCategories: [],
              ageMin: 0,
        ...
        const subsidyRecord = new SubsidyRecord({
          ...
          policyId: subsidy.matchedPolicy?._id || defaultPolicy._id,
          policyName: subsidy.matchedPolicy?.name || "社区助餐补贴政策",
          policyVersion: subsidy.matchedPolicy?.version || 1,
          matchedRule: matchedRuleSnapshot,
          baseSubsidy: subsidy.baseSubsidy,
          seniorSubsidy: subsidy.seniorSubsidy,
          totalSubsidy: subsidy.totalSubsidy,
          mealPrice,
          selfPayAmount: subsidy.selfPayAmount,
          calculationDetail: subsidy.calculationDetail,
          month: monthKey,
          settled: true,
        });
